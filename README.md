...
- **Branch in new chat** — clone any chat's settings (model, MCP tools, crypto, invoke config, ...) into a brand-new chat seeded with history up to the assistant reply you branched from, mirroring ChatGPT's "branch in new chat"; generated and uploaded files referenced in that history are duplicated too — not just linked — so the branch keeps working after the source chat is deleted
